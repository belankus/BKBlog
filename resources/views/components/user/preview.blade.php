        ...
        <div class='flex items-center gap-4'>
            ...
            @if ($details->profile_pic)
                <div @click="window.dispatchEvent(new CustomEvent('show-modal-image', { detail: { userData: {{ $user }}, details: {{ $details }} } }))"
                    class="relative h-32 w-32 rounded-full border-4 border-white bg-cover shadow-lg transition-[filter_500ms_ease] hover:cursor-pointer hover:brightness-90"
                    style="
                    background-image: url('{{ env('APP_URL') }}/storage/{{ $details->profile_pic }}');
                ...
                @else
                    <div @click="window.dispatchEvent(new CustomEvent('show-modal-image', { detail: { userData: {{ $user }}, details: {{ $details }} } }))"
                        class="relative h-32 w-32 rounded-full border-4 border-white bg-cover shadow-lg transition-[filter_500ms_ease] hover:cursor-pointer hover:brightness-90"
                        style="background-image: url('{{ env('APP_URL') }}/img/user_photo.png');">
            @endif
            {{-- Verified Badge --}}
                        ...
                        <div class="flex flex-grow flex-col items-start justify-end gap-4">
                            ...
                            <div class="flex items-center space-x-4">
                                <img class="h-7 w-7 rounded-full"
                                    src="{{ $details->profile_pic ? env('APP_URL') . '/storage\/' . $post->user->details->profile_pic : env('APP_URL') . '/img/user_photo.png' }}"
                                    alt="{{ $user->name }}" />
