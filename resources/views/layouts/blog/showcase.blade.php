                ...
                @foreach ($getRows as $row)
                    <article
                        ...
                        <div class="relative w-full">
                            ...
                            @else
                                <img src="https://source.unsplash.com/300x200?category={{ $row->category->name }}"
                            ...
                            <div class="absolute top-0 flex w-full items-center justify-between p-2 text-gray-500">
                                ...
                                <span
                                    class="rounded bg-black bg-opacity-70 px-2.5 py-0.5 text-sm text-white">{{ Carbon\Carbon::parse($row->published_at)->diffForHumans() }}</span>
                            </div>
                        </div>
                        <div class="relative flex flex-grow flex-col p-6">
                            <ul class="mb-1 flex flex-wrap gap-1">
                                @foreach ($row->tags->sortBy('id')->unique() as $tag)
                                    @php
                                        $queryLast = $row->tags->sortBy('id')->unique()->last();
                                        $queryFirst = $row->tags->sortBy('id')->unique()->first();
                                        $flexGrow = 'flex-grow';
                            ...
                            <div class="flex flex-grow flex-col items-start justify-end gap-4">
                                <a href="/blog/{{ $row->getYear($row->published_at) }}/{{ $row->slug }}"
                                    ...
                                    <span class="">Read in </span><span
                                        class="indent-1">{{ $row->time_to_read }}</span>
